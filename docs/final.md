...
- [4. Architecture Overview](#4-architecture-overview)
  - [4.1. Role Based Access Control](#41-role-based-access-control)
  - [4.2. AS-IS Architecture](#42-as-is-architecture)
    - [4.2.1. Azure Enterprise Scale@Uniper Architecture](#421-azure-enterprise-scaleuniper-architecture)
    - [4.2.2. Azure Enterprise Scale@Uniper – HaCT Cloud Engineer RBAC Architecture](#422-azure-enterprise-scaleuniper-hact-cloud-engineer-rbac-architecture)
    - [4.2.3. Azure Enterprise Scale@Uniper – Application Team RBAC Architecture](#423-azure-enterprise-scaleuniper-application-team-rbac-architecture)
  - [4.3. Application Team – AD Group Naming Convention](#43-application-team-ad-group-naming-convention)
    - [4.3.1. How are Security AD Groups created?](#431-how-are-security-ad-groups-created)
    - [4.3.2. Naming Convention](#432-naming-convention)
  - [4.4. Default Authorization – Application Team](#44-default-authorization-application-team)
    - [4.4.1. RBAC – Application Team](#441-rbac-application-team)
    - [4.4.2. Azure AD Group Ownership and Membership](#442-azure-ad-group-ownership-and-membership)
  - [4.5. Application Team - Role Based Access Control](#45-application-team---role-based-access-control)
    - [4.5.1. Application Support Engineers Default Access](#451-application-support-engineers-default-access)
      - [4.5.1.1. Lower Environment – DEV/SANDBOX](#4511-lower-environment-devsandbox)
      - [4.5.1.2. Upper Environment – UAT/PROD](#4512-upper-environment-uatprod)
  - [4.6. Default Authorization – HaCT Team](#46-default-authorization-hact-team)
    - [4.6.1. HaCT – Cloud Engineer – Default Access](#461-hact-cloud-engineer-default-access)
    - [4.6.2. HaCT – Cloud Engineer - Role Specific Access](#462-hact-cloud-engineer---role-specific-access)
    - [4.6.3. HaCT – Automation Service Principal - Role Specific Access](#463-hact-automation-service-principal---role-specific-access)
    - [4.6.4. Azure AD Role – HaCT Security and IAM Team](#464-azure-ad-role-hact-security-and-iam-team)
    - [4.6.5. Approval Workflow](#465-approval-workflow)
...
- [13. References](#13-references)
  - [13.1. Appendix – A Glossary](#131-appendix-a-glossary)
...
|------|------|
| Architecture ID |  |
| Service / Platform | Azure Enterprise-Scale / PCF V2 |
| Author | Indhumathi Subramanian |
| Reviewers |  |
| Status | Draft |
| Version | 1.0 |
...
### 4.2. AS-IS Architecture

#### 4.2.1. Azure Enterprise Scale@Uniper Architecture
...
#### 4.2.2. Azure Enterprise Scale@Uniper – HaCT Cloud Engineer RBAC Architecture

![Figure 3](images/source_091_figure_3.png)

Figure 2: ES@Uniper - HaCT Cloud Engineer Access

#### 4.2.3. Azure Enterprise Scale@Uniper – Application Team RBAC Architecture
...
### 4.3. Application Team – AD Group Naming Convention

#### 4.3.1. How are Security AD Groups created?
...
### 4.4. Default Authorization – Application Team

#### 4.4.1. RBAC – Application Team
...
Table 11: App Team - Default Role Description

#### 4.4.2. Azure AD Group Ownership and Membership
...
Table 12: AAD Group Member and Owner

*Application Manager can add his application team members into the AD Groups depending on the requirement.

### 4.5. Application Team - Role Based Access Control

#### 4.5.1. Application Support Engineers Default Access

##### 4.5.1.1. Lower Environment – DEV/SANDBOX
...
Table 13: Application Team Access - Lower Environment
...
##### 4.5.1.2. Upper Environment – UAT/PROD
...
Table 14: Application Team Access - Upper Environment
...
### 4.6. Default Authorization – HaCT Team

#### 4.6.1. HaCT – Cloud Engineer – Default Access
...
Table 15: Default Access - HaCT Cloud Engineer
...
#### 4.6.2. HaCT – Cloud Engineer - Role Specific Access
...
Table 16: HaCT Cloud Engineer - Role Specific Access
...
#### 4.6.3. HaCT – Automation Service Principal - Role Specific Access
...
Table 17: HaCT Platform Automation - Service Principal Access
...
#### 4.6.4. Azure AD Role – HaCT Security and IAM Team
...
Table 18: Azure AD Role - HaCT Security and IAM Team

#### 4.6.5. Approval Workflow
...
Table 20: HaCT Stream - Service Owners

## 5. Design Options Considered

- **Option A – AD-group-based RBAC (default Reader/Contributor security AD groups created via IaC) vs individual ad hoc user role assignments**
  - **Why it was considered:** Access management is based on RBAC with Azure AD synced with on-premises AD. During subscription deployment via IaC, “respective AD Groups are created with the standard naming convention pattern and Application Managers are assigned as Owners/Member of AD Groups.” Default Application Team roles include built-in **Reader** and **Contributor** via groups (e.g., `AZ-<Subscription name>-READER`, `AZ-<Subscription name>-CONTRIBUTOR`).
  - **Why it was not chosen (alternative rejected):** Direct/individual role assignments are monitored and removed when they violate standards (e.g., “On noticing role assignments apart from Reader for Application team members in PROD subscription, HaCT Team will be removing the respective role assignment.”). Non-security-type group assignments are also removed (“HaCT team will be removing the role assignment of other AD Group type role assignment except Security type ad groups”).
  - **Key trade-offs:**
    - **Pros of group-based:** standardized naming, automated provisioning, App Manager manages membership, aligns with least privilege.
    - **Cons of group-based:** additional group governance required (ownership/membership management by Application Manager / Directory Service).

- **Option B – PIM-based role-specific access + Four Eye audit approval vs permanently assigned high privilege access**
  - **Why it was considered:** HaCT Cloud Engineers have **Reader** by default at PCFv2 Management Group, and elevated “Role Specific Access” is assigned with **PIM (Yes)** via PAG groups (e.g., `AZ-HaCT-PIM-Security Team`, `AZ-HaCT-PIM-Governance Team`). Access approval follows “Four Eye audit process, i.e two-step approval process” (first approval from HaCT Head and second approval from respective Service Owner).
  - **Why it was not chosen (alternative rejected):** Permanent eligible high privilege access is explicitly avoided in at least one critical case: the Break-Glass account “must not be set to be eligible in Privileged Access Management (PIM), but assigned permanently” (only for emergency access). Additionally, critical/high privilege roles (e.g., “Owner, User Access Administrator, Resource Policy Contributor”) are requested “not use across ESLZ subscriptions/Resource Groups/Resources,” and if identified “during audit process, HaCT Team will removing immediately.”
  - **Key trade-offs:**
    - **Pros of PIM + Four Eye:** time-bound/role-specific elevation and two-step approval aligned to HaCT IAM standard.
    - **Cons of PIM + Four Eye:** manual steps for HaCT Cloud Engineer AD group creation/role assignments and PIM implementation are performed manually by UIT HaCT Security Services; operational overhead vs permanent assignment.

- **Option C – Azure DevOps-based RBAC backup/reporting automation vs no historic RBAC reporting**
  - **Why it was considered:** A “Back-Up Plan” is defined to capture “historic data of all Azure Role Assignments on a particular day” and enable comparison/analysis. Implemented via “PowerShell Script” in an “Azure DevOps pipeline” scheduled daily (Monday–Sunday, 4.00 PM CET) generating an Excel file and uploading to Storage Account `hactsecuritysto001` container `upcfv2roleassignments`.
  - **Why it was not chosen (alternative rejected):** Not having backup/reporting would not meet purposes listed (historic tracking, comparisons, cleanup analysis).
  - **Key trade-offs:**
    - **Pros of automation:** repeatable daily reporting, centralized storage, supports audits/cleanup.
    - **Cons of automation:** requires service principal permissions (Reader at “UPCFv2 Enterprise Scale” MG and Storage Account Contributor at `hactsecuritysto001`, plus Microsoft Graph `Directory.Read.All`).

- **Option D – Emergency Break-Glass account model vs relying solely on normal admin accounts under Conditional Access/Identity Protection**
  - **Why it was considered:** Conditional Access and Azure Identity Protection can “block out users and administrator” in emergencies; break-glass accounts ensure “at least these accounts can log in to the Azure environment.”
  - **Why it was not chosen (alternative rejected):** Relying only on normal admin accounts risks lockout “during unforeseen circumstances such as a natural disaster emergency.”
  - **Key trade-offs:**
    - **Pros of break-glass:** guaranteed emergency access; cloud-only; excluded from Conditional Access.
    - **Cons of break-glass:** highly privileged (Global Administrator), MFA not configured, password never expires; requires shared password handling (split between HaCT Internal members and SMI team) and regular validation/training (at least twice a year) with password changes after training/when dual access cannot be ruled out.

- **Option E – Built-in roles (Reader/Contributor/UAM/Resource Policy Contributor/etc.) vs custom role creation**
  - **Why it was considered:** Default role set is defined using built-in roles for both Application Team and HaCT streams; guidance states “Not to create custom role, in most of the use case Contributor access will be sufficient.”
  - **Why it was not chosen (alternative rejected):** If App team creates custom role, “Custom role is also will be removed” unless there is “valid business justification” post consultation and approval (Service Owner / HaCT Security & IAM). Alerts are also planned for “creation of new custom role.”
  - **Key trade-offs:**
    - **Pros of built-in roles:** standardized, supported, easier governance; aligns with policy/audit controls.
    - **Cons of built-in roles:** may not meet edge-case least-privilege needs; exceptions require consultation and justification.

## 6. Chosen Design & Rationale

- **RBAC chosen to align with least privilege and need-to-know**: RBAC is used to provide fine-grained access management of Azure resources and to restrict access based on “need to know and least privilege security principles”.

- **Standard default access model to reduce privilege and operational overhead**:
  - Application Team default roles rely on built-in roles (e.g., **Reader**, **Contributor**, **User Access Administrator**, **Support Request Contributor**, **Storage Blob Data Reader**) with defined permissions.
  - In **UAT/PROD**, application team members are limited to **Reader** (and **Support Request Contributor**) while application **service principals** receive **Contributor** for deployments; **User Access Administrator** is assigned to the **Application SP** to create/delete role assignments.
  - Critical/high privilege roles (**Owner, User Access Administrator, Resource Policy Contributor**) are requested **not** to be used across ESLZ scopes; if identified during audit, **HaCT will remove immediately**, and if non-Reader access is found for application team members in **PROD**, HaCT will remove it.

- **Azure AD security groups with consistent naming to support scalable, subscription-based access**:
  - During subscription deployment via **IaC**, default security AD groups are created using the standard pattern:
    - `AZ-<Subscription name>-READER`
    - `AZ-<Subscription name>-CONTRIBUTOR`
  - These security AD groups are created **only for the purpose of ES@Uniper RBAC for application team members**.

- **Application Manager ownership/membership model chosen for accountable access management**:
  - Application Managers are configured as **Owner** of both **Reader** and **Contributor** AD groups during subscription deployment.
  - Application Manager is responsible to **grant/revoke access** to application team members by managing membership, and must assess permitted users and provide access based on requirement.

- **Clear separation of responsibilities between Application Team and HaCT**:
  - HaCT creates default AD groups and performs default role assignments on ordered subscriptions.
  - Application Team is responsible for requesting new AD groups beyond default (via UNIPER Directory Service team) and for on-demand role assignments for themselves, using least privilege.

- **PIM used for HaCT role-specific elevated access to limit standing privileges**:
  - HaCT Cloud Engineers have **default Reader** access at **PCFv2 Management Group** to monitor the PCFv2 estate.
  - Elevated role-specific access is provided via **PIM (Yes)** using PAG security AD groups (e.g., **User Access Administrator**, **Resource Policy Contributor**, **Network Contributor**, **Monitoring Contributor**, **Contributor**), supporting activities such as role assignment management, policy deployment/support, network component deployment, and monitoring configuration.
  - In MVP1.0 rollout, AD group creation/role assignments for HaCT Cloud Engineers and **PIM implementation** are performed **manually** by **UIT HaCT Security Services** (uit-hact-security-services@uniper.energy).

- **Monitoring/alerting selected to detect risky RBAC changes and custom roles**:
  - Alerts are defined for **critical role assignments**, **creation of new custom roles**, and **critical resource deployments** across all scopes in UPCFv2, with action group notifications to **UIT HaCT Security Services <uit-hact-security-services@uniper.energy>**.

- **Backup/reporting automation chosen for auditability and operational control of access**:
  - A daily Azure DevOps pipeline executes PowerShell scripts to produce historical reports (Excel) of **Azure role assignments** (and planned for **AD group membership**) and uploads them to **Storage Account: hactsecuritysto001** (container **upcfv2roleassignments** for role assignments).
  - This supports viewing/comparing changes over time, finding role assignments by principal/resource, and cleanup analysis.
  - Schedule: **Monday–Sunday, 4.00 PM CET**.

- **Break-glass accounts included for disaster recovery and emergency access**:
  - Emergency access (“break glass”) accounts are designed to ensure access when Conditional Access/Azure Identity Protection could block administrators (e.g., during network unavailability).
  - They are **highly privileged cloud-only accounts**, excluded from identity-protection services and Conditional Access, with **Global Administrator** permanently assigned (not eligible in PIM), **no MFA**, and password set to **never expire** with at least **16 characters**.
  - Break-glass validation/training is to be performed at least **twice a year**, with password changes when it cannot be ruled out that one individual has access to both password parts.

- **Custom roles minimized to reduce complexity and risk**:
  - Application Team is requested **not to create custom roles**; Contributor is stated as sufficient for most SPN deployment needs.
  - When an app team creates a custom role, it will be removed unless there is valid business justification following consultation with HaCT Security & IAM and involvement of the Service Owner.
...
- **Azure AD / On-premises AD dependency**: Uniper uses Azure AD synced with on-premises Active Directory; RBAC for workers is based on organizational roles.

- **UNIPER Directory Service dependency (manual role enablement)**:
  - HaCT Security and IAM team must reach out to UNIPER Directory Service team to get Azure AD roles assigned.
  - Access requests are placed via ServiceNow catalog: [Directory Service - Request Azure / M365 administrator role assignment or removal](https://uniperprod.service-now.com.mcas.ms/unipersp?id=sc_cat_item_uni&sys_id=8b45c7ebdbc820544ceddc03f3961923) (select **Azure Active Directory (Uniper SE) PROD**).
  - Application Team needs to contact the UNIPER Directory Service team for new AD group creation other than default.

- **IaC/automation dependency for subscription deployment and default RBAC**:
  - Subscriptions are deployed via IaC (owned/supported by HaCT Automation Team); during deployment, respective AD Groups are created using the standard naming convention and Application Managers are assigned as Owners/Member.
  - Detailed explanation about IaC will be covered in the Low level design of **HaCT Platform automation**.

- **Manual actions for HaCT Security Services / MVP1.0 rollout constraint**:
  ...
  - PIM implementation of role specific access for HaCT Cloud Engineers is manually done by **UIT HaCT Security Services** (uit-hact-security-services@uniper.energy).

- **Approval workflow constraint (Four Eye audit process)**:
  - Two-step approval process: first approval from **HaCT Head** (Whillans, Mathew <mathew.whillans@uniper.energy>), second approval from respective **Service Owner**.

- **Operational ownership dependency (Application Manager responsibilities)**:
  - Application Manager is responsible to grant/revoke access to Application Team members on the required subscription and must assess permitted users.
  - HaCT creates default AD Groups, assigns App Manager as Owner of AD Groups (Contributor/Reader) and shares details to App Manager.

- **Monitoring Team dependency / TBD items**:
  - Resource group for placing alert rules (critical role assignments, new custom role creation, critical deployments) is **TBD** and “Monitoring Team has to confirm us.”
  - “Alert rule topic is not completed, Rule name and details are required.”

- **Azure DevOps pipeline + Azure Storage Account dependency (backup plan)**:
  - Backup/reporting is implemented via PowerShell scripts configured via **Azure DevOps pipeline**, scheduled daily (Monday–Sunday, **4.00 PM CET**), generating Excel and uploading to **Azure Storage Account**.
  - Storage Account: [hactsecuritysto001](https://portal.azure.com/); Container: [upcfv2roleassignments](https://portal.azure.com/?feature.msaljs=false).
  - DevOps repo: [RoleAssignments - Repos (azure.com)](https://dev.azure.com/uniperteamservices/HaCT%20Security%20Services/_git/RoleAssignments); Pipeline: [Pipelines - Runs for Azure Role Assignment Backup UPCFv2](https://dev.azure.com/uniperteamservices/HaCT%20Security%20Services/_build?definitionId=6179).
  - Additional backup (AD group membership) has **TBD** items (Storage Container / DevOps Git URL / DevOps Pipeline).

- **Service Principal permissions dependency (RBAC + Microsoft Graph)**:
  - Service Principal used: **Cloud Security Services | PROD | BSN0003595 | General Automation**.
  - Azure RBAC permissions required: **Reader** at “UPCFv2 Enterprise Scale” UPCFv2 Management Group and **Storage Account Contributor** at storage account **hactsecuritysto001**.
  - Azure AD API permissions required: **Microsoft Graph - "Directory.Read.All"**.

- **Constraints on privileged roles/custom roles (audit/removal enforcement)**:
  - Critical/High Privilege roles (“Owner, User Access Administrator, Resource Policy Contributor”) are requested not to be used across ESLZ scopes; if identified during audit, HaCT Team will remove immediately.
  - Custom roles are discouraged; if created, “Custom role is also will be removed” unless valid business justification post consultation with HaCT Security & IAM team and post Service Owner.

- **Design dependency on Governance LLD**:
  - Descriptions about initiatives and policies are explained in the **Governance LLD document**.

## 11. Lifecycle & Evolution Considerations (OPTIONAL)

- **Automation-first for Application Team RBAC**
  - During subscription deployment via IaC, the respective Security AD Groups are created with the standard naming convention, and Application Managers are assigned as Owners/Member of AD Groups.
  - AD Group creation and Role Assignment for application team are automated.

- **Manual rollout and evolution for HaCT Cloud Engineer access (MVP1.0)**
  - AD Group creation and Role Assignments for HaCT Cloud Engineers are performed manually in MVP1.0 roll out by UIT HaCT Security Services (uit-hact-security-services@uniper.energy).
  - PIM implementation of Role Specific access for HaCT Cloud Engineers are manually done by UIT HaCT Security Services (uit-hact-security-services@uniper.energy).

- **Expansion of “critical roles” coverage over time**
...
- **Monitoring/alerting expected to mature (details pending)**
  - Alerts are defined for critical/high privilege role assignments, creation of new custom roles, and critical resource deployments across UPCFv2.
  - Action Group is the Security and IAM Team Distribution List: UIT HaCT Security Services <uit-hact-security-services@uniper.energy>.
  - Alert rule placement Resource Group and other details are **TBD**; “Alert rule topic is not completed, Rule name and details are required”.

- **Ongoing auditability and cleanup enabled via daily backups**
  - **Azure Role Assignments backup**: PowerShell script via Azure DevOps pipeline scheduled **daily (Monday–Sunday, 4.00 PM CET)**; generates an Excel file with current role assignment details on UPCFv2 Management group and uploads to Storage Account **hactsecuritysto001** / container **upcfv2roleassignments**. Purpose includes “Analysis for Role Assignment clean-up”.
  - **AD group membership backup**: PowerShell script via Azure DevOps pipeline scheduled **daily (Monday–Sunday, 4.00 PM CET)**; generates an Excel file with members of AD Groups which have role assignment across PCFv2 and uploads to Storage Account **hactsecuritysto001** (container/pipeline details **TBD**). Purpose includes “Analysis of Members of AD Groups and cleanup.”

- **Break-glass lifecycle (validation and password change trigger)**
  - Break-glass procedure should be trained at least **twice a year**.
...
- Detailed explanation about IaC will be covered in Low level design of HaCT Platform automation.
- Remarks – Description about the Initiatives and Policies are explained in the Governance LLD document.
- HaCT is not responsible for creating the AD Group for Enterprise Scale@Uniper except for the default AD Groups.
- Requesting for new AD group creation other than default: Application Team (R, A).
- Create of role assignment other than default: Application Team (R, A); HaCT Cloud Engineer (C, I).
- Deletion of role assignment other than default: Application Team (R, A); HaCT Cloud Engineer (C, I).
- Access for Application member / Grant/Revoke access to Application team must be taken care by Application Manager.
- App Team's responsible to perform the role assignment for themselves on-demand.
- Custom Role creation: HaCT Team (R, A) / Application Team (C, I); Application Team not to create custom role; when App team creates custom role, Custom role is also will be removed; App Team must place consultation call with HaCT Security & IAM team; in case of valid business justification, Post Service Owner, Application Team will be allowed to create custom role.
- Alerts / Monitoring: Resource Group (where Alert rules are placed, Monitoring Team has to confirm us.) - < TBD > (for critical role assignment, creation of new custom role, deployment alerts).
- [Alert rule topic is not completed, Rule name and details are required]
- Back-Up Plan (Membership of AD group of Application Team): Storage Container: <TBD>; DevOps Git URL: <TBD>; DevOps Pipeline: <TBD>.
